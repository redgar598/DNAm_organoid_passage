...
# p value
mod<-model.matrix(~as.factor(sex), data=epic.organoid)
fit <- lmFit(organoid_Mval, mod)
ebfit <- eBayes(fit)

delta_beta<-sapply(1:nrow(organoid_beta), function(x) {
  group_means<-tapply(organoid_beta[x,], epic.organoid$sex, mean)
  group_means[1]-group_means[2] # F-M
})

limmares_organoid <- data.frame(p.value=ebfit$p.value[,"as.factor(sex)M"], delta_beta=delta_beta)
limmares_organoid$adjusted_p<-p.adjust(limmares_organoid$p.value, method="fdr", n=nrow(limmares_organoid))



organoid_hits<-limmares_organoid[which(limmares_organoid$adjusted_p<=0.05),]
organoid_hits<-organoid_hits[which(abs(organoid_hits$delta_beta)>=0.05),]
nrow(organoid_hits)

organoid_hits_top<-organoid_hits
...
heat_plot_df$X2<-factor(heat_plot_df$X2, levels=epic.organoid$array.id[order(epic.organoid$sex, epic.organoid$passage.or.rescope.no_numeric)])
...
plot_grid(
  ggplot()+geom_tile(aes(X2, 1, fill=sex),heat_plot_df)+theme_void()+
    geom_text(aes(x=c(20, 60), 1, label=c("F","M"))),#+ theme(legend.position = "none")
...
ggsave(file=here("figs", "heat_plot_sex_top.pdf"), w=7, h=5 )
ggsave(file=here("figs/jpeg", "heat_plot_sex_top.jpeg"), w=7, h=5 )
