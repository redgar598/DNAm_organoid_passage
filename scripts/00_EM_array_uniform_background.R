...
em = function(converted, total, prior_H, prior_U, prior_M, prior_I, mu_U, rho_U, mu_M, rho_M, mu_I, rho_I) {	
	...
	while (abs(L - last_L) >= 10 ) { #(abs(0.001 * last_L))
		...
		mu_U = min(max(sum(ratio * membership_probability_U) / sum(membership_probability_U), 0), 1-1e-3)
		var_U = sum((ratio - mu_U)^2 * membership_probability_U) / sum(membership_probability_U)
		rho_U = min(max(var_U / (mu_U * (1-mu_U)), 0), 1-1e-3)
		
		mu_M = min(max(sum(ratio * membership_probability_M) / sum(membership_probability_M), 0), 1-1e-3)
		var_M = sum((ratio - mu_M)^2 * membership_probability_M) / sum(membership_probability_M)
		rho_M = min(max(var_M / (mu_M * (1-mu_M)), 0), 1-1e-3)
		
		mu_I = min(max(sum(ratio * membership_probability_I) / sum(membership_probability_I), 0), 1-1e-3)
		var_I = sum((ratio - mu_I)^2 * membership_probability_I) / sum(membership_probability_I)
		rho_I = min(max(var_I / (mu_I * (1-mu_I)), 0), 1-1e-3)
...
test_em = function(prior_H=0.33, prior_U=0.38, prior_M=0.13, prior_I=0.16, mu_U = .08, rho_U = .03, mu_M = .92, rho_M = .02, mu_I = 0.53, rho_I = .05) {
	# Generate dataset
	total = 1000
	min_count = 100
...
fit_params_varI = function(converted, total, model) {
  fit_L(converted, total, model$prior_H, model$prior_U, model$prior_M, model$prior_I, model$mu_U, model$rho_U, model$mu_M, model$rho_M, model$mu_I, model$rho_I, passage)
}

fit_varI = function(converted, total, prior_H, prior_U, prior_M, prior_I, mu_U, rho_U, mu_M, rho_M, mu_I, rho_I) {
  ratio = converted/total
  likelihood_H = 1 / total
  likelihood_U = dbetabinom(converted, total, mu_U, rho_U)
  likelihood_M = dbetabinom(converted, total, mu_M, rho_M)
